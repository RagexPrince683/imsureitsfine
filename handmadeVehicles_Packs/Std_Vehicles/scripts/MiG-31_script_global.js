				...
				for(i = 0; i < 36;i ++){
					...
					if((i-1)%3 == 0){
						GL11.glPushMatrix();
							...
							GL11.glScalef(-0.001,-0.001,0.001);
							GL11.glTranslatef(-ins.renderFont_getWidth((i+1))/2,-4.5,zdepth);
							ins.renderFont((i+1),0x00FF00,false);
							GL11.glDisable(GL11.GL_TEXTURE_2D);
						GL11.glPopMatrix();
					}
				}
...
function ModelUpdate_Pre(insRender,insParts){
	...
	if(insParts.partsID == 30)RenderVehicle.currentBaseLogic.script_local.invokeFunction("update_canopy_First",insRender,insParts);
	if(insParts.partsID == 31)RenderVehicle.currentBaseLogic.script_local.invokeFunction("update_canopy_Second",insRender,insParts);
	return false;
}
