...
function update_Post(ins){
	ins.script_local.invokeFunction("update_DoorState", ins);
}
...
var GLList_IndYaw = -1;
function GUI_rendering_3D(ins,vehicle){
	if(!vehicle.getBaseLogic().ispilot(Packages.handmadevehicle.HMVehicle.HMV_Proxy.getEntityPlayerInstance()))return false;
...
//		importClass(Packages.handmadeguns.event.HMGEventZoom);
		...
		GL11.glPushMatrix();
			...
			if(GLList_IndPitch == -1){
				...
				if(GLList_IndPitch == -1)return false;
	...
	GL11.glPushMatrix();//コンパス
		...
		GL11.glPushMatrix();
			...
			if(GLList_IndYaw == -1){
				...
				if(GLList_IndYaw == -1)return false;
...
function ModelUpdate_Pre(insRender,insParts){
	...
	if(insParts.partsID == 1)RenderVehicle.currentBaseLogic.script_local.invokeFunction("render_DoorRU",insRender,insParts);
	if(insParts.partsID == 2)RenderVehicle.currentBaseLogic.script_local.invokeFunction("render_DoorLU",insRender,insParts);
	if(insParts.partsID == 3)RenderVehicle.currentBaseLogic.script_local.invokeFunction("render_DoorRD",insRender,insParts);
	if(insParts.partsID == 4)RenderVehicle.currentBaseLogic.script_local.invokeFunction("render_DoorLD",insRender,insParts);
	return false;
}
function missileAnimation(insRender,insParts,insVehicle){
